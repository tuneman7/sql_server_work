...
-- Create the distro_channel table
CREATE TABLE distro_channel (
...
CREATE OR REPLACE FUNCTION update_distro_channel()
RETURNS TRIGGER AS $$
BEGIN
...
CREATE TRIGGER distro_channel_update_trigger
BEFORE UPDATE ON distro_channel
FOR EACH ROW
EXECUTE FUNCTION update_distro_channel();
